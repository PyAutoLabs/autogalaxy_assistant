"""
Galaxy-specific results-inspector tools, layered on the mirrored core
(`tools.py`).

Everything here resolves string specs like "subplot_fit.data" against the
`ag.agg` enum groups and hands them to the existing aggregation machinery
(`af.AggregateImages`, `af.AggregateFITS`) — argument parsing + one call,
nothing more (`skills/ag_inspect_results_mcp.md`, "glue, not code").
"""
...
def _enum_groups() -> dict:
    return {
        name: obj
        for name in dir(ag.agg)
        if not name.startswith("_")
        and isinstance(obj := getattr(ag.agg, name), type)
