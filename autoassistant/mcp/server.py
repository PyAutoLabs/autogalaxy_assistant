"""
The read-only results-inspector MCP stdio server (galaxy edition).
...
PyAutoGalaxy-specific image/FITS extraction (``galaxy_tools``) on top. Run with
``python -m autoassistant.mcp``; client configuration and the design rules live
in ``skills/ag_inspect_results_mcp.md``.
...
# (`autofit.mcp`, covered by PyAutoFit's tests) via internal imports; the galaxy
# `ag.` API surface it wraps lives in `galaxy_tools.py`, which the audit still scans.
import contextlib
import os
import sys
...
with contextlib.redirect_stdout(sys.stderr):
    ...
    from autoassistant.mcp import galaxy_tools

mcp = core_server()


# --- Galaxy layer (this repo's own tools) -----------------------------------


@mcp.tool()
def list_extractable_images() -> dict:
    """
    The galaxy image groups and names that `combine_galaxy_images` /
    `extract_galaxy_fits` accept, e.g. group "subplot_fit" with names "data",
    "model_data", "normalized_residual_map". Pass specs as "group.name".
    """
    return galaxy_tools.list_image_names()


@mcp.tool()
def combine_galaxy_images(
