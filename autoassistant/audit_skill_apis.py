...
the standard aliases (`ag`, `aplt`, `af`, `aa` and the bare module names),
...
This is the mechanical half of the `ag_audit_skill_apis` skill. The skill drives
...
# ---------------------------------------------------------------------------
# API baseline. autogalaxy_assistant is tied to an autogalaxy version; this file
...
# Modules whose public surface we hash. The four library roots carry
# `__version__`; `autogalaxy.plot` is hashed too (it is where the plot API lives)
# but inherits the autogalaxy version.
...
# length-descending so the regex alternation prefers `autogalaxy` over `ag`.
# There is deliberately no `al` alias — PyAutoLens is not part of this
# assistant's stack. Mapping `al` to `autogalaxy` would make stray `al.*` prose
# *validate* (most AG symbols are shared), certifying an import alias this repo
# never emits. Unregistered, such prose is simply not extracted; catching it is
# the residue grep's job, not the symbol resolver's.
# ---------------------------------------------------------------------------
ALIAS_TO_MODULE: dict[str, str] = {
    "aplt": "autogalaxy.plot",
    "af": "autofit",
    "ag": "autogalaxy",
    "aa": "autoarray",
...
# *alias-rooted dotted symbols* (`ag.AnalysisImaging`) against the installed stack.
...
# `ag.AnalysisImaging(...) + ag.AnalysisInterferometer(...)` resolves symbol-by-symbol,
...
DENY_LIST: tuple[IdiomRule, ...] = (
    IdiomRule(
        ...
        #   - constructor calls: ag.AnalysisImaging(...) + ag.AnalysisInterferometer(...)
...
def iter_markdown_segments(text: str) -> Iterable[tuple[str, str]]:
    ...
    INLINE_CODE_RE so we still catch `` `ag.Foo.bar` `` references inside
...
def extract_symbols_code(text: str) -> dict[Symbol, set[str]]:
    ...
    where stale symbols like `ag.Kernel2D` actually execute.
    """
    out: dict[Symbol, set[str]] = {}
...
def _cross_module_candidates(leaf: str) -> list[str]:
    """Look for `leaf` anywhere across the four PyAuto* roots + one submodule level."""
    if leaf in _cross_cache:
        return _cross_cache[leaf]
    hits: list[str] = []
    for mod_name in VERSIONED_MODULES:
...
def select_files(root: Path, scope: str) -> list[Path]:
    ...
    # execute (e.g. a script written against the old `ag.Kernel2D`). Scanned
    # recursively because scripts/ mirrors autogalaxy_workspace's nested layout.
...
def _installation_kind(module_locations: dict[str, str]) -> str:
    ...
    try:
        distribution = importlib.metadata.distribution("autogalaxy")
    ...
    location = module_locations.get("autogalaxy", "")
...
def render_installation_check(check: InstallationCheck) -> str:
    ...
    if check.locations.get("autogalaxy"):
        lines.append(f"  autogalaxy: {check.locations['autogalaxy']}")
    ...
    # and autogalaxy both fail with the same multi-paragraph message —
    ...
    if check.status != "ready":
        lines.extend(
            [
                ...
                "  Otherwise use `skills/ag_setup_environment.md`; do not install into an",
...
def compute_baseline() -> dict:
    ...
    for name in BASELINE_MODULES:
        mod = importlib.import_module(
            name
        )  # already known importable (plot via autogalaxy)
        names = _public_names(mod)
        api_surface[name] = {"hash": _api_hash(mod), "n_symbols": len(names)}

    return {
        "_comment": "API baseline for autogalaxy_assistant - see autoassistant/audit_skill_apis.py "
        "and skills/ag_audit_skill_apis.md. Regenerate with --write-baseline.",
...
def check_version(root: Path) -> int:
    ...
    if not hash_drift:
        ...
        print(
            ...
            f"(autogalaxy {current['versions']['autogalaxy']}, generated {baseline.get('generated')}){note}."
        )
        return 0
...
# cite only live `ag.*` symbols while its `Project:relative/path` source citations
...
def check_citations(root: Path) -> int:
    ...
    def _check_one(rel_file: Path, project: str, cited: str) -> None:
        ...
        n_citations += 1
        if project == "autogalaxy_assistant":
...
def validate_source(text: str) -> tuple[int, list[str], dict[str, str]]:
    ...
    `ag.Kernel2D` or `aplt.FitImagingPlotter` actually crashes. Returns
...
def run_code_gate(*, code: str | None, file: str | None) -> int:
    ...
    if import_failed:
        ...
        print(
            "  Fix the environment first (skills/ag_setup_environment.md); if this is "
