...
(`autofit.mcp.tools`, the ``autofit[mcp]`` extra) and this repo's galaxy layer
(`galaxy_tools.py`).
...
then plants a subplot-fit grid image and a `fit.fits` so the galaxy extraction
tools have real-shaped inputs without paying for a galaxy fit.
"""
...
def output_root(tmp_path_factory):
    ...
    grid_x = 1 + max(position.value[0] for position in ag.agg.subplot_fit)
    grid_y = 1 + max(position.value[1] for position in ag.agg.subplot_fit)
    for marker in root.rglob(".completed"):
        directory = marker.parent
        (directory / "image").mkdir(exist_ok=True)
        grid_name = subplot_filename(ag.agg.subplot_fit.data)
...
def test_list_image_names():
    names = galaxy_tools.list_image_names()
...
def test_resolve_errors():
    with pytest.raises(KeyError, match="Unknown group"):
        galaxy_tools._resolve("no_such_group.data")
    with pytest.raises(KeyError, match="Unknown name"):
        galaxy_tools._resolve("subplot_fit.no_such_name")


def test_combine_images(output_root):
    image = galaxy_tools.combine_images(
        str(output_root),
...
def test_extract_fits(output_root, tmp_path):
    ...
    path = galaxy_tools.extract_fits(
        str(output_root),
