...
def test_install_preflight_reports_ready_environment():
    ...
    assert "autogalaxy:" in process.stdout
...
def test_install_preflight_distinguishes_absent_stack():
    ...
    assert "ag_setup_environment.md" in process.stderr


def test_install_preflight_distinguishes_broken_import(tmp_path):
    for package in ("autonerves", "autoarray", "autofit", "autogalaxy"):
        package_path = tmp_path / package
        package_path.mkdir()
        body = (
            "raise RuntimeError('broken dependency')\n"
            if package == "autogalaxy"
    ...
    assert "autogalaxy import failed: RuntimeError: broken dependency" in process.stderr
