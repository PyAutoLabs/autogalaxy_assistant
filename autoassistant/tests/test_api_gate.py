...
These pin the two failures that motivated the gate — `ag.FitImagingPlotter` and
...
runs under (source `activate.sh` first). Each case imports autogalaxy, so the file is slow
...
GOOD = "import autogalaxy.plot as aplt; aplt.subplot_fit_imaging(fit=fit)"
GOOD_SUBMODULE = "import autofit.jax.pytrees as pytrees"
GOOD_AUTOCONF_SUBMODULE = "import autonerves.dictable"
STALE_PLOTTER = "import autogalaxy as ag; ag.FitImagingPlotter(fit=fit).subplot_fit()"
...
IDIOM_SUM_OPERATOR = (
    "a = ag.AnalysisImaging(dataset=d) + ag.AnalysisInterferometer(dataset=d2)"
)
...
def test_validator_file_mode(tmp_path):
    ...
    script.write_text(f"import autogalaxy as ag\n{STALE_PLOTTER}\n", encoding="utf-8")
...
def test_hook_allows_non_python_command_without_import():
    # Pre-screen must short-circuit (no autogalaxy import) — assert both allow and speed.
...
def test_hook_validates_py_file_argument(tmp_path):
    ...
    good.write_text(f"import autogalaxy.plot as aplt\n{GOOD}\n", encoding="utf-8")
    assert _decision(_run_hook(f"{sys.executable} {good}")) is None

    bad = tmp_path / "bad.py"
    bad.write_text(f"import autogalaxy as ag\n{STALE_PLOTTER}\n", encoding="utf-8")
...
def test_validate_source_reports_broken_stack_once_not_stale():
    ...
    mod._module_cache["autogalaxy"] = None
    mod._import_errors["autogalaxy"] = wall

    n_stale, lines, import_failed = mod.validate_source("ag.Galaxies; ag.Galaxy; ag.FitImaging")

    assert n_stale == 0 and lines == []  # broken stack is never "stale symbols"
    assert list(import_failed) == ["autogalaxy"]  # reported once, not per symbol
    assert len(import_failed["autogalaxy"]) <= 201  # truncated, not the full wall
...
def test_render_installation_check_groups_identical_errors():
    ...
    check = mod.InstallationCheck(
        ...
        errors={name: "Boom: identical wall" for name in ("autofit", "autogalaxy")},
    ...
    assert "autofit, autogalaxy import failed" in text
...
def test_hook_allows_grep_of_pyauto_file_after_a_python_clause(tmp_path):
    ...
    script.write_text(f"import autogalaxy as ag\n{STALE_PLOTTER}\n", encoding="utf-8")
    proc = _run_hook(f'{sys.executable} -c "import yaml" ; grep -c autogalaxy {script}')
    assert _decision(proc) is None
...
def test_hook_allows_grep_piped_into_python(tmp_path):
    ...
    script.write_text(f"import autogalaxy as ag\n{STALE_PLOTTER}\n", encoding="utf-8")
    proc = _run_hook(f'grep -n autogalaxy {script} | {sys.executable} -c "import io"')
    assert _decision(proc) is None
