...
The memory for *this clone* of `autogalaxy_assistant`, and for any science project
...
- `profile.md` — **who you are working with**: background, interaction mode, HPC access
  and authorization. A fact about the *user*, so it carries to their next project; the
  science goal is `state.md`'s, not this file's. It does not exist in a fresh clone; it is
  created on demand from [`_profile_template.md`](./_profile_template.md) (which is why it
  is named here rather than linked).
...
```
2026-05-22-ngc1300-first-imaging-fit.md
2026-05-23-tightened-effective-radius-prior.md
2026-05-24-bulge-disk-decomposition-results.md
```
...
Cross-link every named concept and profile/model using `[[wiki-link]]` slugs (e.g.
`[[Sersic1968]]`, `[[Emsellem1994]]`, `[[bulge-disk-decomposition]]`). The slugs resolve into
`wiki/core/` and — once it is written — `wiki/literature/`; a slug that has no page yet is
still worth recording, because it becomes a link the moment the page lands.
