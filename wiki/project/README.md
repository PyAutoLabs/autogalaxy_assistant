...
A running journal for *this clone* of `autogalaxy_assistant`. Two things live here:

- `profile.md` — **one** living file describing who's working on this
  clone and what they're doing. It does not exist in a fresh clone; it is created on demand
  (which is why it is named here rather than linked). Built up incrementally as the agent picks up cues from
