# PyAutoGalaxy AI Assistant — Galaxy Structure Literature Wiki

This sub-wiki (`autogalaxy_assistant/wiki/literature/`) gives the assistant the broad
scientific context of galaxy structure: how galaxy light is parameterised and fitted, what
the resulting structural parameters mean, and which surveys and instruments produced the
measurements the field is built on. It follows Karpathy's "LLM Wiki" pattern: a compiled,
...
[`skills/ag_ingest_paper.md`](../../skills/ag_ingest_paper.md).
...
- `bibliography/autogalaxy_literature.bib` records canonical citation metadata and keys.
...
**Verification is not optional.** Every citation recorded here — title, lead author, year,
arXiv ID or DOI — must be checked against a public record (arXiv, ADS, the publisher) before
it is written. A fabricated or misattributed citation is the worst artifact a public
assistant can ship, and metadata recalled from memory is wrong often enough to matter. When
an author list cannot be verified in full, record the lead author and "and others" rather
than inventing co-authors.
...
├── entities/                # specific surveys, instruments, catalogues, software
...
| Type        | Folder       | Scope                                                     |
|-------------|--------------|-----------------------------------------------------------|
| Concept     | `concepts/`  | One scientific concept (e.g. the Sersic profile)          |
| Entity      | `entities/`  | One named thing (survey, instrument, catalogue, code)     |
| Sources     | `sources/`   | All paper stubs for one topic, one section per paper      |
| Index/log   | root         | Navigation and provenance                                 |

## Naming

- File names are lowercase kebab-case: `sersic-profile.md`, `cosmos-web.md`,
  `bulge-disk-decomposition.md`.
- One concept per concept page. If a page tries to cover two ideas, split it.
- Source-collection pages are named by topic: `sources/light-profile-fitting.md`.

## Cross-references

Use `[[page-slug]]` for wiki-internal links — for example `[[sersic-profile]]` or
`[[candels]]`. Slugs match the filename without `.md`. A `[[link]]` that has no target file
yet is fine — it marks a future page to write.

Source pages are linked as `[[sources-<topic>]]` (the `sources/` prefix is dropped from the
filename and re-added as a `sources-` slug prefix), and a specific paper section as
...
sources:                 # optional — papers most relevant to this page,
  - arXiv:astro-ph/0204182   # by arXiv ID, DOI, or "Author Year — tag" citation
...
How this concept shows up in the modelling decisions a PyAutoGalaxy user makes.
...
Same idea but the headings are "What it is / Key facts / Why it matters for galaxy structure
/ Key papers / See also". Use entity pages for: surveys (COSMOS-Web, CANDELS, MaNGA),
instruments (JWST, HST), catalogues (the SDSS structural catalogues), missions (Euclid), and
software.
...
**Reference:** arXiv:astro-ph/0204182  (or a DOI/journal reference, and/or "Author Year — title")
...
The `**Canonical BibTeX key:**` line is machine-read: every section that carries a
`**Supports:**` block must declare one, and the key must exist in
`bibliography/autogalaxy_literature.bib`. `python -m autoassistant.literature
validate-citations` (or `make validate-literature-citations`) enforces both.

## Bibliography key convention

Canonical keys are `SurnameYYYY` — the lead author's surname followed by the publication
year, with a trailing lowercase letter (`Bernardi2017a`) only to disambiguate a genuine
collision. Keys are stable: rename one and every source entry that cites it breaks.
...
that is not here, use [`skills/ag_ingest_paper.md`](../../skills/ag_ingest_paper.md). Add the
