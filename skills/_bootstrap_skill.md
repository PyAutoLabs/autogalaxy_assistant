...
- A new `skills/ag_<name>.md` file authored against `skills/_style.md`.
...
different galaxy-structure tasks is worse than three small skills. If the request spans more than
one task, propose a split and ask which to write first.

Also confirm:

- The data type (imaging, interferometer, multi-wavelength, ellipse/isophote).
...
- Whether they have an existing dataset to validate the skill on, or want a synthetic
  one (in which case the new skill should ground its simulation on
...
Orient → Ask → Branch → Combine arc, the five properties (science first, encourage reading,
conversational, composes, records to the project wiki), python-first, source citations as
`<Project>:<path>`.
...
- New light / mass profile or galaxy object → `PyAutoGalaxy`.
...
- Existing usage patterns in `<Project>/test_<package>/` (tests) or
  `autogalaxy_workspace/scripts/` (examples). **The workspace script is the authoritative
  construction idiom** — see the group table in [`../scripts/AGENTS.md`](../scripts/AGENTS.md)
  to find the right one. Never author a recipe with no grounding script behind it.
...
See [`../wiki/README.md`](../wiki/README.md) for the sub-wiki layout and
`../wiki/core/README.md` for the page format. Note `wiki/core/` is read-only outside an
`ag_update_wiki` run: if a new page is genuinely needed, say so and get the user's agreement
before writing into it.

## Step 6 — draft the skill

Save the new skill at `skills/ag_<task>.md`. Follow `_style.md`:
...
```bash
bash "${PYAUTO_BRAIN:-organs/PyAutoBrain}/bin/install.sh" --write-project-discovery autogalaxy_assistant
bash "${PYAUTO_BRAIN:-organs/PyAutoBrain}/bin/install.sh" --check-project-discovery autogalaxy_assistant
```
...
- The request is essentially "run this existing tutorial script for me" — point the
  user at the existing script in `autogalaxy_workspace` and the relevant wiki page, but
  don't wrap it in a new skill.
- The capability is already on the roadmap in `skills/README.md` for a later phase and the
  user does not need it now. Authoring it early is fine when the user's work actually
  requires it — just author it properly, from the grounding scripts listed there.
...
7. Regenerate and check both Claude and Codex adapters; add entry to `skills/README.md` and a row to
