...
`PyAutoLabs/autogalaxy_assistant`, whether the user is working from a personal fork or from a
collaborator clone with direct branch-push access.

This is a **project-workflow** skill, not a library API skill. The output is a draft pull request
...
Confirm the current repository is `autogalaxy_assistant` and inspect its remotes:
...
- **Upstream target** — the remote that points to `PyAutoLabs/autogalaxy_assistant`.
...
- `origin` = `PyAutoLabs/autogalaxy_assistant`
...
- Follow `AGENTS.md` "Agent attribution": name the actual harness and model
  when known. Add a `Co-Authored-By` trailer only when the active harness supplies
  a valid identity. Do not guess a model, provider email, or session identity.
...
Target `PyAutoLabs/autogalaxy_assistant` and its default branch.
...
When the branch already lives on `PyAutoLabs/autogalaxy_assistant`, `--head <branch-name>` is
sufficient.
