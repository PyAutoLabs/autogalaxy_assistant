...
> **`autogalaxy_assistant` is the copilot; a science project is a separate repo.** This repo is
...
> this `autogalaxy_assistant` clone** (cloning it on demand if absent), so there's one source of
...
> this assistant clone; short and descriptive, e.g. `etg_structure`, `jwst_highz_morph`.)
...
> `autogalaxy_workspace` example, `PyAutoReduce`'s scripts/docs, and the `--sample`/`--dataset`
...
### 4. Modeling scripts
> **Modeling scripts?** They live in `scripts/`, normally adapted from `autogalaxy_workspace`.
...
- `scripts/` (the chosen pipeline(s), or empty and adapted from the workspace later)
...
This is a science project created with autogalaxy_assistant. The assistant is the copilot; this
...
Resolve the assistant clone, in order: `$AUTOGALAXY_ASSISTANT` → sibling `../autogalaxy_assistant`
→ else clone `https://github.com/PyAutoLabs/autogalaxy_assistant` into `sources/autogalaxy_assistant/`
...
- Literature: papers specific to this analysis → `wiki/project/bibliography.md`. General
  galaxy-structure concepts will live in the assistant's shared `wiki/literature/` once it is
  written (refer-back); until then keep every citation project-local.
...
[autogalaxy_assistant](https://github.com/PyAutoLabs/autogalaxy_assistant), the PyAutoGalaxy AI
...
Fork or clone this repo and drive it with your own AI assistant: point `$AUTOGALAXY_ASSISTANT`
at a local `autogalaxy_assistant` clone — or just start your agent here and let it clone the
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "sh -c 'P=\"${CLAUDE_PROJECT_DIR:-.}\"; for d in \"$AUTOGALAXY_ASSISTANT\" \"$P/../autogalaxy_assistant\" \"$P/sources/autogalaxy_assistant\"; do [ -n \"$d\" ] && [ -f \"$d/.claude/hooks/validate_pyauto_code.py\" ] && exec python3 \"$d/.claude/hooks/validate_pyauto_code.py\"; done; exit 0'"
          }
        ]
      }
    ]
...
Papers that belong to this project's reference list live here, one `##` section per paper:
canonical BibTeX key, full reference, the concepts it covers, and the claims it supports —
public arXiv/DOI references only, never local PDF paths. Verify every reference against
ADS/arXiv before recording it; a fabricated citation in a paper-companion repo is the worst
possible defect. When the assistant grows its shared `wiki/literature/` sub-wiki, generally
useful papers can be promoted there; promotion is deliberate, never the default.
```
...
1. **Dated journal** → `wiki/project/YYYY-MM-DD-<slug>.md` (the `_template.md` shape:
   ...
   |-----|---------|------|-----------|----------|
   | MGE baseline fit | `python scripts/imaging.py --dataset ngc1300 --seed 42` | 42 | `output/imaging/ngc1300/light_mge/<hash>/` | autogalaxy `<v>`, autofit `<v>`, assistant `<sha>` |
...
- **A collaborator continues the work by forking or cloning the project** and starting their
  own assistant session inside it: the thin `AGENTS.md` resolves an `autogalaxy_assistant` clone
  via refer-back (`$AUTOGALAXY_ASSISTANT` → sibling → clone-on-demand), so they inherit the same
...
order — `$AUTOGALAXY_ASSISTANT` → `../autogalaxy_assistant` → clone the recorded URL on demand into
the gitignored `sources/autogalaxy_assistant/` — then read `skills/` and `wiki/` from there. This
...
- [`../wiki/project/_state_template.md`](../wiki/project/_state_template.md) — the head
  pointer's shape (rewritten each session).
- [`../wiki/project/_template.md`](../wiki/project/_template.md) — the journal-entry shape
  (Work / Collaborate reuse it).
- [`../wiki/project/_results_summary_template.md`](../wiki/project/_results_summary_template.md)
  — the Publish phase's `--notes-file` source.
