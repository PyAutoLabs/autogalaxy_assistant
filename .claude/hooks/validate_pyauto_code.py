...
Two recurring failures motivated this hook: a session generated `ag.FitImagingPlotter`
...
The same script backs both the autogalaxy_assistant hook and the PyAutoLabs-monorepo hook;
...
# Alias-rooted symbol (`ag.`, `aa.`, `aplt.`, `autoarray.` …) or the literal dead
...
def main() -> None:
    ...
    if reports:
        _deny(
            ...
            "(e.g. ag_load_results.md uses `aplt.subplot_fit_imaging(fit=fit)`), or "
