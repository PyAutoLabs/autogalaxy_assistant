# Activate the Python environment that has PyAutoGalaxy installed.
#
# Most users just `pip install autogalaxy` (see the ag_setup_environment skill) into a
# virtual environment. By default this looks for a `.venv` created inside the project
# (what ag_setup_environment makes); edit VENV to point elsewhere, e.g. ~/venv/PyAuto.
#
